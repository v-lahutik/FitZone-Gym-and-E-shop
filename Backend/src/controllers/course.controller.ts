...
export const deleteCourse = async (req:Request, res:Response, next:NextFunction) => {
    try {
      ...
      if (deletedCourse) {
        ...
        return res.status(400).json({ msg: 'There is no such course in our database' });
      }
    } catch (error) {
      next(error);
    }
...
export const addNewCourse = async (
  ...
  try {
    ...
    const newCourse = await Course.create({courseName,category,weekday,date,time,maxParticipants,description,instructor});
